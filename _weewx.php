...
e107::js('_weewx','js/my.js','jquery');	// Load Plugin javascript and include jQuery framework 
e107::css('_weewx','css/my.css');		// load css file
e107::lan('_weewx'); 					// load language file ie. e107_plugins/_weewx/languages/English.php 
...
if($rows = $sql->retrieve('weewx','*',false,'',true)) 	// combined select and fetch function - returns an array. 
{
	// print_a($rows);
	foreach($rows as $key=>$value)		// loop throug
	{
		$text .=  $tp->toHtml($value['weewx_type'])."<br />";		
	}
