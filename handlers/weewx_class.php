...
if ( file_exists( THEME . 'weewx_template.php' ) )
{
    define( WEEWX_THEME, THEME . 'weewx_template.php' );
} else
{
    define( WEEWX_THEME, 'templates/weewx_template.php' );
}
//require_once (WEEWX_THEME);
include_lan( e_PLUGIN . 'weewxfb/languages/' . e_LANGUAGE . '_weewx.php' );
//require_once(e_HANDLER . 'date_handler.php');
//require_once('includes/weewx_shortcodes.php');
...
class weewx
{
    ...
    {
        //  print_a($this->prefs['weewx_wdpath']);
        $path = $this->prefs['weewx_wdpath'] . "testtags.php";

        if ( !is_readable( $path ) )
        {
            print "Unable to locate $path";
            //   $weewxcache = new ecache;
            // $cString = 'menu_weewx_forecast';
            //  $weewxcache->clear($cString);
            //  $cString = 'menu_weewx_current';
            //  $weewxcache->clear($cString);
        } else
        {
            $file = fopen( $path, "r" );
            ...
            while ( !feof( $file ) )
            {
                ...
                if ( strpos( $row, '$' ) === 0 )
                {
                    $tmp = str_replace( $search, "", $row );

                    $tmp2 = explode( "=", $tmp, 2 );
                    $key = trim( $tmp2[0] );

                    $tmp3 = explode( ';', $tmp2[1], 3 );
                    if ( strpos( $tmp2[1], 'on:' ) > 0 )
                    {
                        $value = trim( $tmp3[0] ) . ";" . trim( $tmp3[1] );
                    } else
                    {
                        $value = trim( $tmp3[0] );
                    }
...
Forecast information from <a href="https://www.wunderground.com/?ID=IMAGHULL3">Wunderground</a> at ' . $this->getForecastTime() . '</span>';
