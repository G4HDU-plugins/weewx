 ...
 * e107 weewx Plugin
 *
 * $Source: /cvs_backup/e107_0.8/e107_plugins/weewx/admin_config.php,v $
 * $Revision$
 * $Date$
 * $Author$
 *
...
class plugin_weewx_admin extends e_admin_dispatcher
{
	...
	protected $modes = array(
		'main'		=> array('controller' => 'plugin_weewx_admin_ui',
		'path' 		=> null, 
		'ui' 		=> 'plugin_weewx_admin_form_ui', 'uipath' => null)
	);
	...
	*/
	...
	/**
	 * Format: 'MODE/ACTION' => array('caption' => 'Menu link title'[, 'url' => '{e_PLUGIN}weewx/admin_config.php', 'perm' => '0']);
	 ...
	 * @var string
	 */
	protected $menuTitle = 'weewx Menu';
}
...
{
		// required
		protected $pluginTitle = "e107 weewx";
		 ...
		 * Maybe we need something like $prefs['core'], $prefs['weewx'] ... multiple getConfig support?
		 *
		 * @var string
		 */
		...
		//protected $editQry = "SELECT * FROM #weewx WHERE weewx_id = {ID}";

		// required - if no custom model is set in init() (primary id)
		protected $pid = "weewx_id";
		...
		//TODO change the weewx_url type back to URL before weewx.
		// required
		/**
    	...
    	protected  $fields = array(
			...
			'weewx_id'					=> array('title'=> LAN_ID, 					'type' => 'number',		'data' => 'int',		'width'=>'5%',		'thclass' => '',  'class'=>'center',	'forced'=> TRUE, 'primary'=>TRUE/*, 'noedit'=>TRUE*/), //Primary ID is not editable
           	'weewx_icon'				=> array('title'=> LAN_ICON, 			'type' => 'icon',		'data' => 'str',		'width'=>'5%',		'thclass' => '',	'forced'=> TRUE, 'primary'=>TRUE/*, 'noedit'=>TRUE*/), //Primary ID is not editable          
            'weewx_type'	   			=> array('title'=> LAN_TYPE, 				'type' => 'method', 	'data' => 'str',		'width'=>'auto',	'thclass' => '', 'batch' => TRUE, 'filter'=>TRUE),
			'weewx_folder' 				=> array('title'=> 'Folder', 			'type' => 'text', 		'data' => 'str',		'width' => 'auto',	'thclass' => ''),
			'weewx_name' 				=> array('title'=> 'Name', 				'type' => 'text', 		'data' => 'str',		'width' => 'auto',	'thclass' => ''),
			'weewx_version' 			=> array('title'=> 'Version',			'type' => 'number', 		'data' => 'str',		'width' => 'auto',	'thclass' => ''),
			'weewx_author' 				=> array('title'=> LAN_AUTHOR,			'type' => 'text', 		'data' => 'str',		'width' => 'auto',	'thclass' => 'left'),
         	'weewx_authorURL' 			=> array('title'=> "Url", 				'type' => 'url', 		'data' => 'str',		'width' => 'auto',	'thclass' => 'left'),
            'weewx_date' 				=> array('title'=> LAN_DATE, 			'type' => 'datestamp', 	'data' => 'int',		'width' => 'auto',	'thclass' => '', 'readParms' => 'long', 'writeParms' => 'type=datetime'),
			'weewx_compatibility' 		=> array('title'=> 'Compatible',			'type' => 'text', 		'data' => 'str',		'width' => '10%',	'thclass' => 'center' ),
			'weewx_url' 				=> array('title'=> LAN_URL,		'type' => 'file', 		'data' => 'str',		'width' => '20%',	'thclass' => 'center',	'batch' => TRUE, 'filter'=>TRUE, 'parms' => 'truncate=30', 'validate' => false, 'help' => 'Enter weewx URL here', 'error' => 'please, ener valid URL'),
		...
		protected $fieldpref = array('checkboxes', 'weewx_id', 'weewx_type', 'weewx_url', 'weewx_compatibility', 'options');
		...
		/*protected  $validationRules = array(
			'weewx_url' => array('required', '', 'weewx URL', 'Help text', 'not valid error message')
		);*/
...
class plugin_weewx_admin_form_ui extends e_admin_form_ui
{
	
	function weewx_type($curVal,$mode) // not really necessary since we can use 'dropdown' - but just an example of a custom function.
	{
		$frm = e107::getForm();
		...
		if($mode == 'batch') // Custom Batch List for weewx_type
		{
			return $types;
		}

		if($mode == 'filter') // Custom Filter List for weewx_type
		{
			return $types;
		}

		return $frm->select('weewx_type', $types, $curVal);
	}
 ...
 * Example: $dispatcher = new plugin_weewx_admin(null, null, false);
 */
...
function admin_config_adminmenu() 
{
	//global $rp;
	//$rp->show_options();
	e107::getRegistry('admin/weewx_dispatcher')->renderMenu();
}
*/
